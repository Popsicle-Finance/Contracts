...
/**
 * This contract is made to reward ICE token holder
 * who are willing to stake their $ICE hence supporting 
 * the jump start of popsicle.finance
 * 
 * The contract shall reward 110% of deposited ICE tokens
 * after a one week stake. The contract is for single use only.
 * 
 * The emergency withdrawal is not available in this contract
 * which implies that the user can not withdraw his funds 
 * before the end of the contract
 */

contract DiamondHands is Ownable, ReentrancyGuard {
    ...
    /**
     * @dev Throws if called when deposits are closed.
     */
    ...
    function withdraw() external nonReentrant {
        ...
        if (pendingTokenReward > 0) { // save some gas
            uint256 withdrawAmount = safeRewardTransfer(msg.sender, pendingTokenReward);
            user.withdrawedAmount += withdrawAmount;
    ...
    function safeRewardTransfer(address _to, uint256 _amount) internal returns(uint256) {
        ...
        if (_amount > rewardTokenBalance) {
            iceToken.transfer(_to, rewardTokenBalance);
            return rewardTokenBalance;
        }
        iceToken.transfer(_to, _amount);
        return _amount;
    }
