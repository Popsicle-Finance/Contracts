...
contract DiamondHands is Ownable {
    ...
    uint256 week = 10 minutes;
    
    IERC20 private iceToken;
    
    event Deposit(address indexed user, uint256 amount);
    event Withdraw(address indexed user, uint256 amount);
    
    struct UserInfo {
        uint256 amount; // How many Ice tokens the user has provided.
    ...
    function pendingReward(address beneficiary) public view returns(uint256) {
        ...
        return userAmount.div(10).add(userAmount.mul(2)).sub(user.alreadyWithdrawn);
    }
    ...
    function withdraw() external {
        ...
        if (pendingTokenReward > 0) {
            uint256 withdrawAmount = safeRewardTransfer(msg.sender, pendingTokenReward);
            user.alreadyWithdrawn = withdrawAmount.add(user.alreadyWithdrawn);
